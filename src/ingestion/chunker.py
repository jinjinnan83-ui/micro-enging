...
@dataclass
class TextChunk:
    """A retrieval unit that always carries Bandura metadata."""
...
def _split_long_text(text: str, size: int, overlap: int) -> list[str]:
    """Split a paragraph that is itself longer than the window."""
    if len(text) <= size:
        return [text]
    ...
    return _window_paragraphs(sentences, size=size, overlap=overlap)
    ...
    for paragraph in paragraphs:
        if len(paragraph) > size:
            if buffer:
                windows.append("\n\n".join(buffer))
                buffer, buffer_len = [], 0
            windows.extend(_split_long_text(paragraph, size=size, overlap=overlap))
            continue
