"""Hybrid retrieval engine for the Bandura knowledge base.
...
such as 自我效能 and 掌握经验. BGE-Reranker-v2-m3 then reranks the
...
# Longest-first so 自我效能 is not split into 自我 + 效能.
DOMAIN_TERMS = (
...
TERM_ALIASES = {
    "自我效能": ("自我效能", "self-efficacy", "效能预期"),
    "self-efficacy": ("自我效能", "self-efficacy", "效能预期"),
    "掌握经验": ("掌握经验", "亲身掌握", "表现成就", "mastery experience", "performance accomplishments"),
    "mastery experience": ("掌握经验", "亲身掌握", "表现成就", "mastery experience", "performance accomplishments"),
    "performance accomplishments": ("掌握经验", "亲身掌握", "表现成就", "mastery experience", "performance accomplishments"),
    "替代经验": ("替代经验", "观察学习", "vicarious experience"),
    "vicarious experience": ("替代经验", "观察学习", "vicarious experience"),
    "社会劝说": ("社会劝说", "言语劝说", "social persuasion", "verbal persuasion"),
    "言语劝说": ("社会劝说", "言语劝说", "social persuasion", "verbal persuasion"),
    "social persuasion": ("社会劝说", "言语劝说", "social persuasion", "verbal persuasion"),
    "生理状态": ("生理状态", "情绪状态", "physiological states"),
    "physiological states": ("生理状态", "情绪状态", "physiological states"),
    "引导掌握": ("引导掌握", "参与示范", "guided mastery", "participant modeling"),
    "guided mastery": ("引导掌握", "参与示范", "guided mastery", "participant modeling"),
    "近端目标": ("近端目标", "proximal goal"),
    "proximal goal": ("近端目标", "proximal goal"),
    "三元交互": ("三元交互", "相互决定", "reciprocal determinism"),
    "reciprocal determinism": ("三元交互", "相互决定", "reciprocal determinism"),
    "道德疏离": ("道德疏离", "moral disengagement"),
    "moral disengagement": ("道德疏离", "moral disengagement"),
}

SORTED_TERMS = tuple(sorted(DOMAIN_TERMS, key=len, reverse=True))


def tokenize_bandura(text: str) -> list[str]:
...
def expand_query_tokens(tokens: list[str]) -> list[str]:
    """Add theoretical aliases so 掌握经验 also matches mastery experience."""
...
class HybridEngine:
    ...
    def _bm25_search(self, question: str) -> list[RetrievalHit]:
        if self._bm25 is None or not self._corpus:
            return []
        tokens = expand_query_tokens(tokenize_bandura(question))
    ...
    def _rebuild_bm25(self) -> None:
        ...
        tokenized = [tokenize_bandura(row.get("text", "")) for row in self._corpus]
...
def index_documents(
    ...
    *,
    ...
    """Index Bandura chunks into Qdrant (bge-large-zh-v1.5) and refresh BM25."""
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(description="Index or query the Bandura hybrid engine.")
    ...
    query_cmd.add_argument("question", help="例如：什么是自我效能 / 掌握经验")
